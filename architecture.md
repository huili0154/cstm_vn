...
cstm_vn 是一个 **ETF 量化回测与实盘交易系统**。核心功能包括：

- 从 7z 压缩包中提取原始 Tick CSV 数据，清洗后转为 Parquet 格式存储
- 通过 TuShare API 下载 ETF 日线行情及复权因子
- 基于 PyQt5 的交互式数据浏览器，支持日线/Tick 表格查看、时序图表绘制、数据导出
- **回测引擎**：三种撮合模式（日线收盘价 / Tick 即时 / 延迟深度撮合），统一策略接口
- **实盘引擎**：接入 mini-QMT（xtquant），策略代码回测/实盘零修改运行，含 PyQt5 交易 GUI

技术栈：Python 3.10+ / PyQt5 / pandas / pyarrow / pyqtgraph / py7zr / tushare / xtquant
...
```
cstm_vn/
├── core/                        # 核心数据类型与策略接口（回测/实盘共用）
│   ├── __init__.py
│   ├── datatypes.py             # Direction, OrderType, OrderStatus, Order, Trade,
│   │                            #   Position, Account, TickData, BarData, MatchingMode
│   ├── data_feed.py             # ParquetBarFeed / ParquetTickFeed 数据源
│   ├── event.py                 # 事件枚举 (预留)
│   ├── matching.py              # MatchingEngine — 三种撮合模式实现
│   └── strategy.py              # EngineBase (抽象接口) + StrategyBase (策略基类)
│
├── backtest/                    # 回测引擎
│   ├── __init__.py
│   ├── engine.py                # BacktestEngine(EngineBase) — 回测主循环
│   └── report.py                # BacktestReport — 绩效统计与图表
│
├── live/                        # 实盘交易引擎（mini-QMT）
│   ├── __init__.py
│   ├── __main__.py              # python -m live 入口
│   ├── qmt_gateway.py           # QmtGateway — xtquant 适配层
│   ├── engine.py                # LiveEngine(EngineBase) — 实盘引擎
│   └── trader_gui.py            # TraderWindow — PyQt5 交易 GUI
│
├── gui_viewer/                  # 桌面 GUI 数据浏览器
│   ├── __init__.py
...
│
├── tests/                       # 测试用例
│   ├── test_matching.py         # MatchingEngine 单元测试
│   ├── test_backtest_engine.py  # BacktestEngine 测试
│   ├── test_integration.py      # 集成测试
│   └── test_smart_sell_observation.py
│
...
backtest/engine.py (BacktestEngine)
    ├── core/strategy.py          # EngineBase + StrategyBase
    ├── core/datatypes.py         # 所有数据类型
    ├── core/matching.py          # MatchingEngine (撮合)
    └── core/data_feed.py         # ParquetBarFeed / ParquetTickFeed

live/engine.py (LiveEngine)
    ├── core/strategy.py          # EngineBase + StrategyBase
    ├── core/datatypes.py         # 所有数据类型
    └── live/qmt_gateway.py       # QmtGateway → xtquant SDK
            └── xtquant            # mini-QMT Python API

live/trader_gui.py (TraderWindow)
    ├── live/engine.py
    ├── live/qmt_gateway.py
    └── core/datatypes.py
```
...
StrategyBase (策略基类)
    ↓ self._engine
    ├── BacktestEngine   ← 回测: 撮合引擎 + Parquet 数据源
    └── LiveEngine       ← 实盘: QMT 网关 + 实时行情
```
...
### 4.0a core/ — 核心数据类型与策略接口

回测引擎和实盘引擎共享的基础设施。

**core/datatypes.py — 数据类型定义**

| 类型 | 说明 |
|------|------|
| `Direction` | BUY / SELL |
| `OrderType` | LIMIT / MARKET |
| `OrderStatus` | SUBMITTING → ACTIVE → PART_TRADED → ALL_TRADED / CANCELLED / REJECTED |
| `MatchingMode` | CLOSE_FILL / TICK_FILL / SMART_TICK_DELAY_FILL |
| `Order` | 委托 (order_id, symbol, direction, price, volume, traded, status) |
| `Trade` | 成交 (trade_id, order_id, symbol, price, volume, commission) |
| `Position` | 持仓 (symbol, volume, frozen, cost_price, today_bought; T+1 限制) |
| `Account` | 账户 (balance, frozen, commission) |
| `TickData` | Tick 快照 (58 列, 10 档盘口) |
| `BarData` | 日线/分钟线 (OHLCV + 后复权价) |

**core/strategy.py — 策略接口**

- `EngineBase` (ABC): 回测引擎和实盘引擎必须实现的 9 个抽象方法
  - `send_order`, `cancel_order`, `cancel_all`, `get_pending_orders`
  - `get_position`, `get_account`, `write_log`, `get_pricetick`, `get_latest_tick`
- `StrategyBase` (ABC): 策略基类, 提供生命周期回调和下单便捷方法
  - 回调: `on_init`, `on_start`, `on_stop`, `on_day_begin`, `on_tick`, `on_bar`, `on_order`, `on_trade`
  - 下单: `buy`, `sell`, `buy_market`, `sell_market`, `cancel_order`, `cancel_all`

**core/matching.py — 撮合引擎**

三种模式:
- **CLOSE_FILL**: 以日线收盘价全量成交，适合日频策略
- **TICK_FILL**: 价格触及 bid1/ask1 即全量成交，不考虑盘口深度
- **SMART_TICK_DELAY_FILL**: 延迟 ≥1 tick + 10 档逐档吃单 + 被动排队 + 部分成交

**core/data_feed.py — 数据源**

- `ParquetBarFeed`: 读取 `dataset/daily/` 日线 Parquet
- `ParquetTickFeed`: 读取 `dataset/ticks/` Tick Parquet

### 4.0b backtest/ — 回测引擎

**backtest/engine.py — BacktestEngine(EngineBase)**

实现 EngineBase 全部接口，驱动主循环：

```
on_init → on_start → [per trading day:
    on_day_begin(prev_bar) → per tick: on_tick(tick) → matching
] → on_stop
```

核心功能:
- 资金/持仓前检（买入冻结资金, 卖出冻结持仓）
- 品种合法性检查（symbol 不在订阅列表则 REJECT）
- 日净值记录 (daily_nav)
- T+1 买入限制

**backtest/report.py — BacktestReport**

- `print_summary()`: 输出收益率、最大回撤、夏普比率等
- `show_charts()`: matplotlib 绘制净值曲线

### 4.0c live/ — 实盘交易引擎

**live/qmt_gateway.py — QmtGateway**

mini-QMT (xtquant) 适配层，封装所有与 QMT SDK 的交互:

| 方法 | 功能 |
|------|------|
| `connect() / disconnect()` | 连接/断开 xtdata + XtQuantTrader |
| `subscribe(symbols)` | 订阅实时 tick (subscribe_whole_quote) |
| `send_order(...)` | 下单 → `trader.order_stock()` |
| `cancel_order(order_id)` | 撤单 → `trader.cancel_order_stock()` |
| `query_account()` | 查询资金 → Account |
| `query_positions()` | 查询持仓 → dict[str, Position] |
| `query_orders() / query_trades()` | 查询当日委托/成交 |

数据转换映射:
- QMT tick (5 档 askPrice/bidPrice 数组) → TickData (10 档, 6-10 补零)
- QMT order_status (48~57) → OrderStatus (SUBMITTING~REJECTED)
- QMT order_type (STOCK_BUY=23/STOCK_SELL=24) → Direction
- 回调桥: `XtQuantTraderCallback` → `on_order_cb` / `on_trade_cb` / `on_tick_cb`

**live/engine.py — LiveEngine(EngineBase)**

实盘引擎，实现 EngineBase 接口:
- `start(strategy)`: 连接网关 → 注册回调 → on_init → on_start → 订阅行情
- `stop()`: 取消订阅 → on_stop
- Tick 推送: primary symbol (symbols[0]) 触发 `strategy.on_tick()`
- 委托/成交: QMT 回调 → 更新 `_active_orders` → `strategy.on_order/on_trade()`
- 线程安全: `threading.Lock` 保护活跃委托字典

**live/trader_gui.py — TraderWindow**

PyQt5 交易 GUI:
- **连接面板**: QMT 路径 + 账号, 一键连接/断开
- **策略面板**: 策略类选择 + 品种配置 + JSON 参数 + 启动/停止
- **手动下单**: 品种/方向/价格/数量, 限价/市价下单
- **实时表格**: 行情 (涨跌着色) / 持仓 (浮盈着色) / 委托 / 成交
- **账户信息**: 总资产 / 可用 / 冻结 / 市值 (1秒定时刷新)
- **撤单**: 选中撤单 / 全撤
- **日志窗口**: 所有引擎日志实时显示
- 跨线程安全: `_SignalBridge` (pyqtSignal) 桥接 QMT 回调线程 → GUI 主线程
...
└────────┬────────┘    └─────────┬─────────┘             │
         ...
         └───────────┬───────────┘                       │
              ...
              │       │                                      │
              │       ├── ParquetBarFeed  ← dataset/daily/   │
              │       ├── ParquetTickFeed ← dataset/ticks/   │
              │       └── StrategyBase (策略代码)              │
              └──────────────────────────────────────────────┘
              ...
              │          live/ (实盘引擎)                      │
              │                                              │
              │  LiveEngine ── QmtGateway ── xtquant SDK     │
              │       │              │                       │
              │       │              ├── xtdata (行情)        │
              │       │              └── XtQuantTrader (交易) │
              │       └── StrategyBase (同一份策略代码)         │
              │                                              │
              │  TraderWindow (PyQt5 交易GUI)                 │
              └──────────────────────────────────────────────┘
              └──────────────────────────────────────────────┘
```
...
# ── 实盘交易 GUI ──
python -m live
python -m live --qmt-path "C:\QMT\userdata_mini" --account 1860049999
...
| matplotlib | 回测报告图表 |
| xtquant | mini-QMT Python SDK（实盘交易） |
